...
class GeneratePDFService {
    execute({ proposal, company, address, responsible, head, cnpj, phone, email, investment, titles, descriptions }) {
        if (titles === undefined || descriptions === undefined)
            throw new Error("Por favor, crie ao menos um componente");
        const doc = new jspdf_1.default();
        let X = 10;
        let Y = 32;
        doc.setFontSize(12);
        function setBold(set, orange) {
            if (set) {
                if (orange) {
        ...
        function formatHeader(str, info, x) {
            setBold(true, true);
            doc.text(`${str}:`, X, Y);
            setBold(false);
            doc.text(info, X + x, Y);
        ...
        function formatItem(title, description, orange) {
            setBold(true);
